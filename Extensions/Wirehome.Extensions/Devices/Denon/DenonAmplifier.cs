...
namespace Wirehome.Extensions.Devices
{
    ...
    public class DenonAmplifier : DeviceComponent, IDisposable
    {
        ...
        {
            _fullState = await _eventAggregator.PublishWithResultAsync<DenonStatusMessage, DenonDeviceInfo>(new DenonStatusMessage { Address = Hostname }).ConfigureAwait(false);
            var mapping = await _eventAggregator.PublishWithResultAsync<DenonMappingMessage, DenonDeviceInfo>(new DenonMappingMessage { Address = Hostname }).ConfigureAwait(false);
